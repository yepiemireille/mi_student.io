...
//ATTENTION??::vous devez désactiver la sécurité de votre compte d'ou vous envoyer l'email parce que sinon PHPMailer ne va pas pouvoir accéder à votre compte
	...
	if(isset($_POST['valideretud_email'])){
		$email = $_POST['email'];
		// selectionne tous ce qui est dans la table teacher ou email = ?
		$requete_sur_email = $conn->prepare('SELECT * FROM etud WHERE email = ?');
...
// ************************************************************
		...
		}else{echo "<center style='color:red;'>L'email est invalide (n'existe pas dans le site)</center>";}
	}
			...
			<div class="card">
				...
				<div class="card-body">
					<form method="post">
						...
						<div class="form-group">
							<input type="submit" name="valideretud_email" id="valideretud_email" class="btn btn-info" value="Envoie de mail" />
						</div>
					</form>
					<?php 
					if(isset($error)){
